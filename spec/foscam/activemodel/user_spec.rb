...
describe Foscam::ActiveModel::User do
  ...
  #     Foscam::ActiveModel::User.client = @client
  #     users = Foscam::ActiveModel::User.all
  ...
  # end
	...
	describe ".create" do
		it "builds a user and then saves it" do
			Foscam::ActiveModel::User.any_instance.stub(:save).and_return(true)
			Foscam::ActiveModel::User.any_instance.should_receive(:save).once
			user = Foscam::ActiveModel::User.create(valid_params)
		...
		context "when user exists" do
			context "with valid id" do
				it "returns the desired user by id" do
					@client.should_receive(:get_params).once
					Foscam::ActiveModel::User.client = @client
					user = Foscam::ActiveModel::User.find(1)
					user.should be_an_instance_of(Foscam::ActiveModel::User)
					user.id.should == 1
		...
		context "when user doesn't exists" do
			context "with valid id" do
				it "returns a nil object" do
					@client.should_receive(:get_params).once
					Foscam::ActiveModel::User.client = @client
					user = Foscam::ActiveModel::User.find(2)
					user.should be_nil
				end
			end
			context "with invalid id" do
				it "returns a nil object" do
					...
					Foscam::ActiveModel::User.client = @client
					user = Foscam::ActiveModel::User.find(10)
					user.should be_nil
				end
			end
		end		
	...
	describe ".delete" do
		context "with valid id" do
			it "deletes the desired user" do
				...
				Foscam::ActiveModel::User.client = @client
				flag = Foscam::ActiveModel::User.delete(1)
				flag.should be_true
			end
		end
		context "with invalid id" do
			it "deletes the desired user" do
				...
				Foscam::ActiveModel::User.client = @client
				flag = Foscam::ActiveModel::User.delete(10)
	...
	describe "#save" do
		context "a new user" do
			before(:each) do
				@user = Foscam::ActiveModel::User.new(valid_params)
			end
	...
	describe "#==" do
		...
		it "returns false if it is an instance of Foscam::Model::User but the ids are different" do
			user2 = Foscam::ActiveModel::User.new(valid_params)
		...
		it "returns false if either or both ids is not set" do
			user2 = Foscam::ActiveModel::User.new({:username => "user_1", :password => "pass_1", :privilege => :administrator})
			flag = @user == user2
			flag.should be_false
			@user.instance_variable_set(:@id, 1)
			flag = @user == user2
